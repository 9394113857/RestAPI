# 1.This framework is for throwing Errors of Fields:-
# 2.Flask framework and Mysql Database:-
# 3. Get ip and device name from socket library:-
# importing module
import logging
import socket

from faker import Faker
from flask import request, Flask
from flask_mysqldb import MySQL

# Create and configure logger
logging.basicConfig(filename="F:\Restful-API's\RestAPI\log files\Apilogs.log",
                    format='%(asctime)s %(message)s',
                    filemode='a')

# Creating an object
logger = logging.getLogger()

# Setting the threshold of logger to DEBUG
logger.setLevel(logging.DEBUG)

# Test messages
logger.info('-----------------------------')
logger.info("api_logs script started Now:-")
"""
logger.debug("Harmless debug Message")
logger.info("Just an information")
logger.warning("Its a Warning")
logger.error("Did you try to divide by zero")
logger.critical("Internet is down")
"""

# Flask App Initialization:-
app = Flask(__name__)

app.config['MYSQL_HOST'] = 'localhost'
app.config['MYSQL_USER'] = 'root'
app.config['MYSQL_PASSWORD'] = 'raghu'
app.config['MYSQL_DB'] = 'clinicalfirst'

mysql = MySQL(app)


# API_Logs:-
# create in postman by using jsonify:-
@app.route('/api_logs/create', methods=['POST'])
def api_logs():
    if 'api_id' in request.json and 'api_name' in request.json \
            and 'error_code' in request.json:

        api_id = request.json['api_id']
        api_name = request.json['api_name']
        error_code = request.json['error_code']

        # Cursor:-
        cursor = mysql.connection.cursor()
        cursor.execute('SELECT * FROM api_logs WHERE API_ID = % s', (api_id,))
        account = cursor.fetchone()
        if account:
            msg = 'Account already exists !'

        # Checking conditions:-
        elif not api_id or not api_name or not error_code:
            msg = 'Please fill out the fields !'

        else:
            cursor = mysql.connection.cursor()

            # UserId Pattern:-
            cursor.execute("SELECT USER_ID FROM api_logs")
            lastid = cursor.rowcount
            print('----------------------')
            print("Last Id is: " + str(lastid))
            lastid += 1
            pattern = 'US000'  # pattern = ooo
            # pattern += 1 # pattern incremnting always by 1:-
            user_id = pattern + str(lastid)
            # User Id pattern Code End #

            # Python Program to Get IP Address and Device Name:-
            device = socket.gethostname()
            example = Faker()
            ipaddress = example.ipv4()

            # Insert Code:-
            cursor.execute(
                "insert into api_logs(API_ID, USER_ID, API_NAME, ERROR_CODE, USER_IP, USER_DEVICE) "
                "VALUES(%s,%s,%s,%s,%s,%s)", (api_id, user_id, api_name, error_code, ipaddress, device))
            mysql.connection.commit()
            # details = cur.fetchall()
            logger.info("successfully registred")
            return "successfully inserted", 200
        return msg
    return "invalid parameters"


# MAIN app:-
if __name__ == "__main__":
    app.run(debug=True)

################################################ END CODE ##############################################################

# Post Man:-
"""
Working URL Now:- Inserting Values
POST:-
http://127.0.0.1:5000/api_logs/create 
Body---> Raw----> json
{
    "api_id" :     1,
    "api_name" :   "Raghu",
    "error_code" : "404"
}
"""

"""
Commented Code:-
'''
        # These are the conditions:-
        elif not re.match(r'[^@]+@[^@]+\.[^@]+', regid):
            msg = 'Invalid email address !'
        elif not re.match(r'[A-Za-z0-9]+', userid):
            msg = 'Username must contain only characters and numbers !'
        elif not re.match(r'^(?:1[01][0-9]|120|1[7-9]|[2-9][0-9])$', userage):   # userage code saved
            # "^(?:1[01][0-9]|120|1[7-9]|[2-9][0-9])$"gm
            msg = 'Age must contain number between 17 to 120 !'
        '''
"""
